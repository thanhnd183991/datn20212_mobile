...
document.getElementById("userAvatarId").src = "http://localhost:8081" + avatarURL;
function loadContent(data){
    ...
    for(var i = 0; i < data.length; i++){
        ...
        var getdiff = (new Date()) - (new Date(data[i].postingDate));
        var days = getdiff/ (1000 * 60 * 60 * 24);
        if(days > 30){
            p_post__topInfo.innerText = data[i].postingDate.substring(0, 10);
        //p_post__topInfo.innerText = date.toLocaleDateString("ën-US", options);
        }else if(days > 1){
            p_post__topInfo.innerText = Math.floor(days).toString() + " days ago";
        }
        else{
            var hours = (getdiff/ (1000 * 60 * 60)) % 23;
            if(hours > 1){
                p_post__topInfo.innerText = Math.floor(hours).toString() + " hours ago";
            }else{
                var minus = (getdiff/ (1000 * 60)) % 60;
                p_post__topInfo.innerText = Math.floor(minus).toString() + " minutes ago";
            }
        }
...
//        <p>Like</p>`;
        ...
        console.log("URL: " + "http://localhost:8081"+avatarURL);
...
function reactPost(id){
    url = 'http://localhost:8081/api/private/post/react/'+id;
    console.log(url);
    fetch(url, {
            method: 'POST',
            headers: {
...
function updateComment(target,data){
    ...
    .then((user) => {
        ...
        if(user){
            ...
            const img_user__avatar_mess_1 = document.createElement('img');
            img_user__avatar_mess_1.classList.add('user__avatar_mess');
            img_user__avatar_mess_1.src = user.profile_Image.uri ;
...
function loadHome(auth){
    fetch('http://localhost:8081/api/private/home-page', {
        method: 'GET',
        headers: {
        'Authorization' : auth,
        'Accept': 'application/json',
