...
public class ActiveUserManager {
    ...
    public Set<String> getActiveFriendsOfCurrentUser(String username) {
        Set<String> users = new HashSet<>(map.keySet());
        Set<String> rs = new HashSet<>(users);
        users.remove(username);
        rs.remove(username);
        for (String element : users) {
            FriendState friendState = friendService.getFriendState(username, element);
            if(friendState.getState() != 1){
                rs.remove(element);
            }
        }
        return rs;
